...
export default {
  ...
  customers: {
    authenticate(account) {
      ...
      let endpoint_custom =
        config.scheme + config.endpoint + config.plugin_endpoint;
  ...
  orders: {
    create(args) {
      ...
      if (
        args.cart &&
        args.cart.shipping_line_items &&
        args.cart.shipping_line_items.length > 0
      ) {
  ...
  pages: {
    get(id) {
      let url = config.scheme + config.endpoint + "/wp-json/wp/v2/pages/" + id;

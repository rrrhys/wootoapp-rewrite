...
const { PRODUCTS_IN_CATEGORY_LOADED, VARIATION_BY_PRODUCT_ID_LOADED, PRODUCT_LOADED } = Actions;
...
const products = (state = initialState, action: productActionTypes): IProducts => {
	...
	switch (action.type) {
		case VARIATION_BY_PRODUCT_ID_LOADED:
			...
			const { product_id, variations: thisVariations } = action as productVariationSuccessAction;
			if (product_id) {
